...
Description :
This is a ColdBox event handler for our RSS generator.

----------------------------------------------------------------------->
<cfcomponent name="examples" extends="coldbox.system.EventHandler" output="false">
	...
	<cffunction name="index" access="public" returntype="void" output="false">
		...
		<cfset Event.setView("generator/vwIndex")/>
	</cffunction>
...
<!------------------------------------------------ EXAMPLE FEEDS ------------------------------------------------>
  ...
  <!--- createFeedBeginner start --->
	<cffunction name="createFeedBeginner" access="public" returntype="void" output="false" hint="">
		...
		outputFile is the name and path of the XML file the plug-in will create, any existing file will automatically be overwritten.
		 --->
		<cfset rc.compileFeed = getModel( "feedGenerator@cbfeeds" ).createFeed(FeedStruct=rc.feed,OutputFile=rc.pathtofile)/>
		<!--- Display the feed using the vwPostGenerate view located at /views/generator/vwPostGenerate.cfm --->
		<cfset Event.setView("generator/vwPostGenerate")/>
	</cffunction>
  ...
  <!--- createFeedSimpleArt start --->
	<cffunction name="createFeedSimpleArt" access="public" returntype="void" output="false" hint="">
		...
		<cfif rc.feed.items.RecordCount gte 1>
			<cfset rc.compileFeed = getModel( "feedGenerator@cbfeeds" ).createFeed(FeedStruct=rc.feed,ColumnMap=rc.map,OutputFile=rc.pathtofile)/>
		<cfelse>
  ...
  <!--------------------------------------------- DUMP EVENTS --------------------------------------------->
	...
	<cffunction name="dumpFeedBeginner" access="public" returntype="void" output="false">
		...
		<cfset Event.setView("generator/vwDumpCOM")/>
	</cffunction>
  ...
  <!--------------------------------------------- DUMP LAYOUTS & VIEWS -------------------------------------------->
	...
	<cffunction name="dumpLayoutAutoDiscovery" access="public" returntype="void" output="false">
		...
		<cfset Event.setView("generator/vwDumpLO")/>
	</cffunction>
	...
	<cffunction name="dumpvwPostGenerate" access="public" returntype="void" output="false">
		<cfargument name="Event" type="any">
		<cfset var rc = Event.getCollection()/>
		<cfset rc.LOName = "generator/vwPostGenerate"/>
		<cfset rc.DumpLO = dumpLayout('views/generator/vwPostGenerate')/>
		<cfset Event.setView("generator/vwDumpLO")/>
	</cffunction>
  ...
  <!-------------------------------------------------- DUMP FEEDS ------------------------------------------------->
	...
	<cffunction name="sourceFeedBeginner" access="public" returntype="void" output="false">
		...
		<cfset Event.setView("generator/vwDumpFeed")/>
	</cffunction>
  ...
  <!--- createFeedDisplayBeginner start --->
	<cffunction name="displayFeedBeginner" access="public" returntype="void" output="false">
		...
		<cfset rc.DumpXML = fileRead('#getSetting("ApplicationPath")#includes/xml/RSS_Sample_Beginner.xml')/>
		<!--- Render the View vwDisplayFeed --->
		<cfif not len(rc.DumpXML)>
			<cfset runEvent(event='generator.viewMissingFeed',private=true)/>
		<cfelse>
			<cfset runEvent(event='generator.viewDisplayFeed',private=true)/>
		</cfif>
	</cffunction>
  <!--- createFeedDisplayBeginner end --->
	...
	<cffunction name="dumpLayout" access="private" returntype="string" output="false">
		...
		<cfscript>
			...
			readF = fileRead('#getSetting("ApplicationPath")##arguments.layout#.cfm');
			readF = replace(readF,chr(9)," ","all"); // remove whitespace TABs from text
			// replace characters to make CFML code visable in HTML
			readF = replaceNoCase(readF,'<','&lt;','all');
			readF = reReplaceNoCase(readF,'(/\*(.|[\r\n])*?\*/)','<span class="scrcomments">\1</span>','all');
	...
	<cffunction name="readXMLfile" access="private" returntype="string" output="false">
		...
		<cfset var readF = ""/>
		<cftry>
			<cfset readF = fileRead('#getSetting("ApplicationPath")#includes/xml/RSS_Sample_#rc.COMname#.xml')/>
	...
	<cffunction name="viewMissingFeed" access="private" returntype="void" output="false">
		...
		<cfset var rc = Event.getCollection()/>
		<!--- Display the feed --->
		<cfset Event.setView("generator/vwMissingFeed")/>
	</cffunction>
